...
{
    Mat image;
    image = imread("/home/zhou/tag36h11/tag36_11_00000.png");

    // 新建一个要合并的图像
    Mat img_merge;
    Size size(46, 1198);
    img_merge.create(size, CV_MAKETYPE(image.depth(), 3));
    img_merge = Scalar::all(255);

    int count = 0;
    string ImgName;
    ifstream fin("/home/zhou/tag36h11/files.txt");//打开原始样本图片文件列表
    while (getline(fin, ImgName)) //逐行读取文件列表
    {
        cout << "processing：" << ImgName << endl;
        ImgName = "/home/zhou/" + ImgName;
        Mat temp;
        temp = imread(ImgName);//读取图片
        int s = 10;
        resize(temp, temp, Size(s,s),0,0,INTER_NEAREST);
        int i = count / 4;
        int j = count % 4;
        temp.copyTo(img_merge(Rect(j*(s+2), i*(s+2), s, s)));
        count ++;
        if(count == 400) break;
    }

    resize(img_merge, img_merge, Size(460,11980),0,0,INTER_NEAREST);
    namedWindow("img_merge", 0);
    imshow("img_merge", img_merge);
    imwrite("tag_test.bmp", img_merge);
    waitKey(0);
    return 0;
}
